...
def render_page(documentation: DocumentationPage, *, with_menu: bool = True) -> None:
    ...
    if with_menu:
        with ui.left_drawer() \
                .classes('column no-wrap gap-1 mt-[-20px] px-8 py-20') \
                .style(' background-color: #5579C6; color: #ffffff; height: calc(100% + 20px) !important') as menu:
    ...
    ui.page_title('Reusable U!' if not title else title if title.split()[0] == 'Reusable U!' else f'{title} | Reusable U!')
    ...
    with ui.column().classes('w-full p-8 lg:p-16 max-w-[1600px] mx-auto')\
                .style(' background-color: #5579C6; color: #ffffff'):
